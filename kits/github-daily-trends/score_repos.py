...
# Keywords and weights tuned for Hermes / Agent OS relevance
RELEVANCE = {
    # Core agent stack
...
def integration_block(repo: dict, relevance: int, rank: int) -> str:
    ...
    return "\n".join([
        ...
        "**Integration:**",
        f"```bash",
        f"# Als externes Tool clonen",
        f"git clone {repo['url']} ~/.hermes/external/{short}",
        f"# SKILL.md schreiben und als Hermes-Skill registrieren",
        f"# cp ~/.hermes/external/{short}/README.md als Basis nutzen",
        f"```",
        "",
    ])
